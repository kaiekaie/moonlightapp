...
socket.on("hello" ,(data) => {
  ...
  socket.on("list", function (data) {
    ...
    data.forEach((element, i) => {
      $("#flat ul").append(`<li  data-flip-category="Games" data-index="${i}" data-value="${element.game}"  >${hasimages(element)}</li>`);
    });
    ...
    myFlipster = $("#flat").flipster({
      ...
      onItemSwitch: function (crr, prv) {
        ...
        if (!$(crr).find("#game").hasClass("active")) {
          console.log()
          let index = $(crr).data("index");
          socket.emit("sendControll", index);
  ...
  let GameViewFunction = function () {
    ...
    socket.on("controllFunction", function (e) {

myFlipster.flipster('jump', e);
      // if (e === "left") {
      //   myFlipster.flipster('prev');
      // } else {
      //   myFlipster.flipster('next');
